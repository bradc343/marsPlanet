...
var material1 = new THREE.LineBasicMaterial( { color: 0xffffff, wireframe: false } );

const line = new THREE.Line( geometry1, material1 );
line.position.set(10, 1, 50);
scene.add( line );
...
// document.addEventListener('mousemove', tiltCamera, false)
// document.addEventListener('mousemove')
document.addEventListener('mousewheel', adjustCamera, false)

// function tiltCamera( event ) {
...
function adjustCamera(event){
  ...
  camera.fov = Math.max(Math.min(camera.fov, forMax), forMin);
  console.log(camera)
  camera.updateProjectionMatrix()
